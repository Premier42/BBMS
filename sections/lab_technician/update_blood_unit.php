...
$unit_id = $donor_id = $blood_type = $volume = $status = "";
$unit_id_err = $donor_id_err = $blood_type_err = $volume_err = $status_err = "";
$success_message = "";

// Get the blood unit details for updating
if (isset($_POST['unit_id'])) {
    $unit_id = $_POST['unit_id'];
    $sql = "SELECT * FROM blood_units WHERE unit_id = ?";
    if ($stmt = $conn->prepare($sql)) {
        ...
        if ($stmt->execute()) {
            ...
            if ($result->num_rows == 1) {
                ...
                echo "Error: Blood unit not found.";
                exit;
            }
        } else {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update_unit'])) {
    ...
    if (empty($donor_id_err) && empty($blood_type_err) && empty($volume_err) && empty($status_err)) {
        $sql = "UPDATE blood_units SET donor_id = ?, blood_type = ?, volume = ?, status = ? WHERE unit_id = ?";
        if ($stmt = $conn->prepare($sql)) {
            $stmt->bind_param("isssi", $param_donor_id, $param_blood_type, $param_volume, $param_status, $param_unit_id);
            ...
            $param_unit_id = $unit_id;
            if ($stmt->execute()) {
                $success_message = "Blood unit updated successfully.";
            } else {
                echo "Something went wrong. Please try again later.";
            }
            $stmt->close();
...
<body>
    <div class="container mt-5">
        <h2 class="mb-4">Update Blood Unit</h2>
        <?php 
